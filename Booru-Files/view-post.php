...
<html>
	<head>
		<?php
			...
			$richtext = array('[b]','[/b]','[i]','[/i]','[br]','[t]');
			$richrepl = array('<b>','</b>','<i>','</i>','<br />','	');
		...
		<div id="content">
			<?php
				...
				elseif($filetype=='mp3'or$filetype=='flac'){
					...
					echo "<img id='image' src=\"$imagedir/$filename\" type=\"image/$rowtype[type]\" alt=\"$titleext\" title=\"$titleext\"/><br />";
				}
				if(strlen($row['description'])>1){
					$description = str_ireplace($richtext,$richrepl,$row['description']);
					echo "<div id='descbox'>Description:<br/>$description</div>";
				}
